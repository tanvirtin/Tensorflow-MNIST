...
if __name__ == "__main__":
    ...
    # argmax used from neural network will be used to calculate accuracy
    logits_layer_argmax = tf.argmax(logits, 1)
            ...
            for start, end in zip(range(0, len(train_x), 128), range(128, len(train_x) + 1, 128)):
                ...
                if counter % 100 == 0:
                    ...
                    '''
                        ...
                        Following our example, tf.equal(tf.argmax(pred, 1),tf.argmax(y, 1)) returns a tensor whose evaluation will give an array(1,1,1,1,1,1).

                    '''

                    a = np.mean(np.argmax(test_y, axis=1) == sess.run(logits_layer_argmax, feed_dict={X: test_x}))
